    ...
    <div class="mb-6 flex flex-col gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
            <a href="{{ route('partner.profile') }}"
               class="inline-flex items-center justify-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:border-amber-400 hover:bg-amber-50 transition-colors">
            ...
            @if($sessions->count() > 1)
                <form action="{{ route('partner.profile.sessions.revoke-all') }}" method="POST" onsubmit="return confirm('Are you sure you want to revoke all other sessions? You will be logged out from all other devices.');" class="w-full sm:w-auto">
                    @csrf
                    <button type="submit"
        ...
        @else
            <div class="divide-y divide-slate-100">
                @foreach($sessions as $session)
                    <div class="p-4 sm:p-6 hover:bg-slate-50 transition-colors {{ $session['is_current'] ? 'bg-amber-50/50' : '' }}">
                        <div class="flex flex-col gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-1.5">
                                        ...
                                        @if($session['is_current'])
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-700 border border-amber-200 w-fit">
                                                Current Session
                                            </span>
                                        @endif
                                    </div>
                            ...
                            <div class="flex items-center justify-end sm:justify-start pt-2 border-t border-slate-100 sm:border-t-0 sm:pt-0">
                                @if(!$session['is_current'])
                                    <form action="{{ route('partner.profile.sessions.revoke', $session['id']) }}" method="POST" onsubmit="return confirm('Are you sure you want to revoke this session?');" class="w-full sm:w-auto">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                ...
                                @else
                                    <span class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-lg">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
