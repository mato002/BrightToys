...
@section('partner_content')
    <div class="mb-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-lg font-semibold">Manage Projects</h1>
                <p class="text-xs text-slate-500">
                    Create and manage projects. Assign users to run each project.
                </p>
            </div>
            <a href="{{ route('partner.projects.manage.create') }}"
                        ...
                        <tr class="hover:bg-slate-50/60">
                            ...
                            <td class="px-4 py-3 align-top text-xs text-slate-600">
                                @if($project->assignedUsers->count() > 0)
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($project->assignedUsers->take(3) as $user)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded bg-slate-100 text-[10px] text-slate-700">
                                                {{ $user->name }}
                                                <span class="ml-1 text-slate-400">({{ $user->pivot->role }})</span>
                                            </span>
                                        @endforeach
                                        @if($project->assignedUsers->count() > 3)
                                            <span class="text-[10px] text-slate-400">+{{ $project->assignedUsers->count() - 3 }} more</span>
                                        @endif
                                    </div>
                                @else
                                    <span class="text-[11px] text-slate-400 italic">No users</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 align-top text-right text-xs">
                                <div class="flex flex-wrap justify-end gap-1">
                                    ...
                                    @endif
                                    <form action="{{ route('partner.projects.manage.destroy', $project) }}" method="POST"
                                          onsubmit="return confirm('Delete this project?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="inline-flex items-center px-2 py-1 rounded bg-red-500 hover:bg-red-600 text-white">
                                            Delete
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-sm text-slate-500">
                                ...
                                <a href="{{ route('partner.projects.manage.create') }}" class="text-amber-600 hover:text-amber-700 underline ml-1">
