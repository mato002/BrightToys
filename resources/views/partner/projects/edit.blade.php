...
@section('partner_content')
    <div class="mb-4">
        ...
        <p class="text-xs text-slate-500">Update project details and user assignments.</p>
    </div>

    <div class="bg-white border border-slate-100 rounded-lg p-6">
        <form action="{{ route('partner.projects.manage.update', $project) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="type" name="type" value="{{ old('type', $project->type) }}" required
                           class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500">
                    @error('type')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                        ...
                        <p class="text-[11px] text-amber-700">Add the URL when the project is developed and ready. You can leave these empty for projects still in planning.</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url', $project->url) }}"
                           ...
                           class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500">
                    <p class="text-[10px] text-slate-500 mt-1">Add when project is live</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="route_name" name="route_name" value="{{ old('route_name', $project->route_name) }}"
                           ...
                           class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500">
                    <p class="text-[10px] text-slate-500 mt-1">Add when route is created</p>
                </div>
            ...
            <div class="pt-4 border-t border-slate-200">
                <h3 class="text-sm font-semibold text-slate-900 mb-3">Assigned Users</h3>
                <p class="text-xs text-slate-500 mb-4">Manage users assigned to this project.</p>
                
                @if($users->count() > 0)
                    <div id="user-assignments" class="space-y-2">
                        @php
                            $assignedUserIds = $project->assignedUsers->pluck('id')->toArray();
                        @endphp
                            ...
                            <div class="user-assignment-item flex items-center gap-2 p-3 border border-slate-200 rounded-lg">
                                <select name="assigned_users[]" class="flex-1 border border-slate-200 rounded px-3 py-2 text-sm">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ $assignedUser->id === $user->id ? 'selected' : '' }}>
                                            {{ $user->name }} ({{ $user->email }})
                                        </option>
                                    @endforeach
                                </select>
                                <select name="user_roles[]" class="border border-slate-200 rounded px-3 py-2 text-sm">
                                    <option value="manager" {{ $assignedUser->pivot->role === 'manager' ? 'selected' : '' }}>Manager</option>
                                    <option value="editor" {{ $assignedUser->pivot->role === 'editor' ? 'selected' : '' }}>Editor</option>
                                    <option value="viewer" {{ $assignedUser->pivot->role === 'viewer' ? 'selected' : '' }}>Viewer</option>
                                </select>
                                <button type="button" onclick="this.closest('.user-assignment-item').remove()" class="text-red-500 hover:text-red-600 text-sm">×</button>
                            </div>
                        @endforeach
                        <div class="user-assignment-item flex items-center gap-2 p-3 border border-slate-200 rounded-lg" style="display: none;">
                            <select name="assigned_users[]" class="flex-1 border border-slate-200 rounded px-3 py-2 text-sm">
                                <option value="">Select a user...</option>
                                @foreach($users as $user)
    ...
    <script>
        function addUserAssignment() {
            ...
            const template = container.querySelector('.user-assignment-item[style*="display: none"]').cloneNode(true);
