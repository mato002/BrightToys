...
@section('page_title', 'Edit Chart of Account')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
            ...
            <h1 class="text-lg font-semibold text-slate-900">Edit Chart of Account</h1>
            <p class="text-xs text-slate-500">Update account information</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.accounting.chart-of-accounts.update', $chartOfAccount) }}" method="POST" class="bg-white border border-slate-100 rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $chartOfAccount->code) }}" required
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $chartOfAccount->name) }}" required
                       class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-400"
            ...
            <div>
                ...
                <select name="type" required
                    ...
                    @foreach($accountTypes as $type)
                        <option value="{{ $type }}" {{ old('type', $chartOfAccount->type) == $type ? 'selected' : '' }}>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="border border-slate-200 rounded w-full px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-400"
                          placeholder="Optional description for this account">{{ old('description', $chartOfAccount->description) }}</textarea>
            </div>
