...
@section('content')
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-4">
        ...
        <div class="flex gap-2">
            <button onclick="createAccount()" class="bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold px-4 py-2 rounded">
                Create
            </button>
            <button onclick="editAccount()" class="bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold px-4 py-2 rounded">
                Edit Acc
            </button>
            <button onclick="removeAccount()" class="bg-red-500 hover:bg-red-600 text-white text-xs font-semibold px-4 py-2 rounded">
                Remove
            </button>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="bg-white border border-slate-100 rounded-lg p-4">
            <h2 class="text-sm font-semibold text-slate-900 mb-3">Accounting Rules</h2>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="{{ $loop->even ? 'bg-slate-50' : 'bg-white' }}">
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button onclick="editAccount({{ $rule->debit_account_id }})" class="text-emerald-600 hover:text-emerald-700">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-3 py-4 text-center text-slate-500 text-xs">
    ...
    <script>
        function createAccount() {
            alert('Create Account functionality - to be implemented');
        }
        ...
        function removeAccount() {
            if (confirm('Are you sure you want to remove the selected account?')) {
                alert('Remove Account functionality - to be implemented');
            }
        }
    </script>
    @endpush
