...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
            ...
            <p class="text-xs text-slate-500">Update admin account information.</p>
        </div>
    ...
    <form action="{{ route('admin.admins.update', $admin) }}" method="POST" class="bg-white border border-slate-100 rounded-lg p-4 text-sm space-y-4 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="border-t border-slate-100 pt-4">
            <h3 class="text-xs font-semibold text-slate-700 mb-3">Change Password (Optional)</h3>
            <p class="text-[10px] text-slate-500 mb-3">Leave blank if you don't want to change the password.</p>
            
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-semibold mb-1 text-slate-700">New Password</label>
                    <div class="relative">
                ...
                <div>
                    <label class="block text-xs font-semibold mb-1 text-slate-700">Confirm New Password</label>
                    <div class="relative">
                ...
                @foreach($roles ?? [] as $role)
                    <label class="flex items-center">
                        <input type="checkbox" name="roles[]" value="{{ $role->id }}" 
                               {{ $admin->adminRoles->contains($role->id) ? 'checked' : '' }}
